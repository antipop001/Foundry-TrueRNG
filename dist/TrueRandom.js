...
export class TrueRandom {
    ...
    Notify(level, message) {
        ...
        try {
            ...
            if (!notifications || typeof notifications[level] !== "function")
                return false;
    ...
    EnsureQuickToggleStyle() {
        if (document.getElementById("TrueRandomQuickToggleStyle"))
            return;
     ...
     */
    GenerateQuickToggleButton(visible) {
        if (!this.IsGM)
            return;
    ...
    UpdateRandomNumbers() {
        if (!this.Enabled || this.AwaitingResponse || !this.RandomGenerator)
            return;
    ...
    OnApiSuccess() {
        ...
        this.PendingApiFailureNotice = false;
        if (this.HasNotifiedLowQuota || !this.IsGM || !this.RandomGenerator)
            return;
        ...
        if (!outOfRequests && !bitsNearlyGone)
            return;
...
TrueRandom.RetryDelayMs = 30000;
/** Ceiling for the failure backoff. */
TrueRandom.MaxRetryDelayMs = 300000;
